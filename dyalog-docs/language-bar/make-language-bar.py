    ...
    for entry in toc_entries:
        ...
        if "link" not in entry:
            if entry.get("title", "").strip().lower() == "blank":
                ...
                symbols.append(
                    SymbolData(title=entry.get("title", ""), char="", content="")
                )
            continue
        ...
        xml_title = (
            ...
            if head is not None and head.find("title") is not None
            else ""
        )
        xml_char = (
            head.find("char").text
            if head is not None and head.find("char") is not None
            else ""
        )
        ...
        pre_content = (
            body.find("pre").text
            if body is not None and body.find("pre") is not None
            else ""
        )
